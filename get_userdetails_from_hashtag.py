...
'''
get user details through hashtag and save int in the ftp
Save info into ftp
'''
...
bot.login(username="1nsta_bot003", password="Insta12#")
...
def get_user_info(user_id):
	...
	sUrl = "http://i.instagram.com/api/v1/users/" + user_id + "/info/"
	rUrl = session.get(sUrl) # Response URL
...
def handle(hashtag):
	...
	for i in range(len(users)):
		...
		if (resp == '429') or (resp=='400'):
			...
			full_name = get_item_json_resp(resp,'full_name')
	...
	output_filename = hashtag + "_" + date_time + ".csv"
...
read_ftp_file ('hashtags','l30artwork_hashtags.txt')
